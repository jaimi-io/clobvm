...
import (
	...
	"github.com/jaimi-io/clobvm/client"
...
func TestE2e(t *testing.T) {
	...
	ginkgo.RunSpecs(t, "clobvm e2e test suites")
}
...
var _ = ginkgo.BeforeSuite(func() {
	...
	for _, u := range uris {
		...
		clobvmRPCEps = append(clobvmRPCEps, rpcEP)
		outf("{{blue}}avalanche clobvm RPC:{{/}} %q\n", rpcEP)
	}
...
var _ = ginkgo.Describe("[ProposeBlock]", func() {
	...
	ginkgo.It("get genesis block", func() {
		for _, inst := range instances {
			...
			gomega.Ω(data).Should(gomega.Equal(chain.BytesToData([]byte("e2e"))))
	...
	data := chain.BytesToData(hashing.ComputeHash256([]byte("test")))
	now := time.Now().Unix()
